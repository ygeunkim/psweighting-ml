...
logit_asam %>% 
  ...
  kable(
    format = "latex",
    col.names = c("Scenarios", "Logistic", "RandomForests", "SVM (Linear)", "SVM (Radial)"),
    escape = FALSE
  ) %>% 
...
#   merge(iptw_rf[,-"iptw"] %>% setNames(c(col_name, "randomForest")), by = col_name) %>%
#   merge(iptw_svm_lin[,-"iptw"] %>% setNames(c(col_name, "svmLinear")), by = col_name) %>% 
#   merge(iptw_svm[,-"iptw"] %>% setNames(c(col_name, "svmRadial")), by = col_name) %>%
#   melt(id.vars = col_name, variable.name = "model", value.name = "PS")
```
...
## Commens about Propensity Scores
...
# col_name <- names(mc_list)
# iptw_dat <- 
#   iptw_logit[,-"propensity"] %>% 
#   setNames(c(col_name, "glm")) %>% 
#   merge(iptw_rf[,-"propensity"] %>% setNames(c(col_name, "randomForest")), by = col_name) %>%
#   merge(iptw_svm_lin[,-"propensity"] %>% setNames(c(col_name, "svmLinear")), by = col_name) %>% 
#   merge(iptw_svm[,-"propensity"] %>% setNames(c(col_name, "svmRadial")), by = col_name) %>%
#   melt(id.vars = col_name, variable.name = "model", value.name = "IPTW")
```
